...
struct Grid final {
    ...
    Grid(const T& val): data(static_cast<Grid<T, dim - 1>*>(operator new(sizeof(Grid<T, dim - 1>)))), sz(1) {
        try {
            new(data) Grid<T, dim - 1>(val);
        } catch (...) {
...
struct Grid<T, 1> final {
    ...
    Grid(const T& val): data(static_cast<T*>(operator new(sizeof(T)))), sz(1) {
        try {
            new(data) T(val);
        } catch (...) {
            operator delete(data);
